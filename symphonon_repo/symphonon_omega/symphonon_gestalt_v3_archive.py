...
DISS_T_THR  = 0.002          # T_locale sotto cui il gestalt si dissolve
DISS_FRAMES = 120            # frame in zona povera prima di dissolversi
MERGE_R     = 3              # fusione solo da quasi sovrapposti
MERGE_PH    = math.pi / 3.5 # soglia fase per fusione gestalt
# SuperGestalt
SG_R        = 18    # zona intermedia
SG_PH_THR   = math.pi / 3.0  # soglia coerenza di fase per accoppiamento
SG_MIN_FRAMES = 12  # frame di coerenza sostenuta
...
class Gestalt:
    ...
    vitality: float = 0.7
    T: float = 0.0
    poor_frames: int = 0  # frame in zona T povera
...
class Simulation:
    ...
    def step(self):
        ...
        # Target ricalibrato: T naturale del campo è 0.05-0.20
        # crit_t precedente (0.003-0.030) era 10× troppo basso
        crit_t = max(0.04, min(0.15, 0.06 + 0.08*(1-coh)))
        T_std  = float(np.std([p.T for p in self.poles])) if self.poles else 0
        T_sig  = self.o_t*0.5 + T_std*0.5
        # Control loop non-Markoviano — risponde a T attraverso o(t)
        # Quando T→0: T_sig→0, crit_t−T_sig>0 → noise_gain sale
        # Guadagno proporzionale alla distanza dalla soglia critica
        # Control loop: risposta proporzionale allo scarto dal target
        err = crit_t - T_sig
        self.noise_gain = max(0.50, min(1.80, self.noise_gain + err * 0.08))
        ...
        # 10. Rilevamento SuperGestalt (ogni 3 frame — finestra temporale più stretta)
        if self._frame % 3 == 0:
        ...
        for i, seed in enumerate(free):
            ...
            if dG < 0.15:
                print(f"[REJECT_dG] frame={self._frame} coh={coh:.3f} dG={dG:.3f} "
                      f"n_poles={len(cluster)} — passa COH, scartato da ∂(G)")
                continue
    ...
    def _step_gestalts(self):
        ...
        for g in self.gestalts:
            ...
            # Vitalità (più lenta dei poli — l'organismo è più grande)
            g.vitality = max(0, min(1, g.vitality - VIT_DECAY*0.5 + VIT_GAIN*0.4*g.T))

            if g.T < DISS_T_THR:
                g.poor_frames += 1
            else:
                g.poor_frames = max(0, g.poor_frames - 2)

            # Dissoluzione
            if g.poor_frames > DISS_FRAMES or g.vitality < VIT_DEATH*0.5:
    ...
    def _step_supergestalts(self):
        ...
        for sg in self.supergestalts:
            ...
            if sg.coh < 0.3:
                for g in members: g.sg_id = -1; g.sg_coh_frames = 0
                sg.dead = True; continue
    ...
    for i, ga in enumerate(members):
        for gb in members[i+1:]:
            ...
            for k in range(N+1):
                ...
                pts.append((bx,by))
            if len(pts) >= 2:
                lw = max(1, int(2*sg.coh))
                c_dim = (col_sg[0]//2, col_sg[1]//2, col_sg[2]//2)
                pygame.draw.lines(surf, c_dim, False, pts, lw)

    # Anello sincronizzato sui gestalt membri
    pulse = 0.7 + 0.3*math.sin(t*1.2 + sg.ph)
    ring_col = (min(255,col_sg[0]+40), min(255,col_sg[1]+40), min(255,col_sg[2]+40))
    for g in members:
        sx,sy = cam.world_to_screen(g.gx, g.gy)
        r_sg = int(g.r_vis * cam.scale/CAM_INIT_SCALE * 1.6 * pulse)
        if r_sg > 2:
            pygame.draw.circle(surf, ring_col, (sx,sy), r_sg, 1)
...
def main():
    ...
    while running:
        ...
        lines = [
            ('SYMPHŌNŌN Ω · GESTALT v3', (100,160,130)),
